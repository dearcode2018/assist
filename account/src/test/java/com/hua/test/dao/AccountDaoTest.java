...
public class AccountDaoTest extends BaseDaoTest {
	...
	public void testSearch() {
		try {
			...
			final AccountSearchBean searchBean = new AccountSearchBean();
			searchBean.setStatus(1);
			
			pager = accountDao.search(pager, searchBean);
			
			System.out.println(pager.getBeanList().size());
	...
	public void testOutputHtml() {
		try {
			...
			// 注意要加上
			String str = MarkerUtil.getTemplateContent("account.ftl", dataMap);
			
			File file = new File(ProjectUtil.getAbsolutePath("/doc/html/account.html", true));
			FileUtil.writeAsString(file, str);
		} catch (Exception e) {
			log.error("testOutputHtml =====> ", e);
		}
	}	
